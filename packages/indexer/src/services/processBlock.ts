...
export async function processBlock(ws: string, connection: Connection) {
    ...
    const metrics = new MetricsService(indexerServer, 3051 + Math.floor((Math.random() * (100 - 0) + 0)), "nodle_indexer_processor_");
...
async function consume(blockNum: number, api: ApiPromise, connection: Connection, metrics: MetricsService, msg: ConsumeMessage, channel: Channel) {
